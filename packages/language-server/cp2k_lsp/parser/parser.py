    ...
    def parse(self) -> CP2KInput:
        ...
        while not self.match(TokenType.EOF):
            if self.match(TokenType.SECTION_START):
                ...
                self.errors.append(
                    SyntaxError(f"Unexpected token: {self.current().value}", self.current().line, self.current().column, self.source)
                )
                self.advance()
    ...
    def parse_section(self) -> Section:
        ...
        while not self.match(TokenType.SECTION_END, TokenType.EOF):
            if self.match(TokenType.SECTION_START):
                ...
                self.errors.append(SyntaxError(f"Unclosed section &{section.name}", start_token.line, start_token.column, self.source))
                break
            else:
                self.errors.append(
                    SyntaxError(
