...
class TestXyRangeFix:
    """Test X..Y range parsing fix for issue #72"""

    def test_global_config_exists(self):
        """Test KEEP_RANGE_AS_STRING global config exists"""
        assert isinstance(KEEP_RANGE_AS_STRING, bool)

    def test_range_as_string_default(self):
        """Test X..Y range returns string by default (issue #72 fix)"""
        result = kw_converter_int('1..10')
        assert isinstance(result, str)
        assert result == '1..10'

    def test_range_as_integer_range_explicit(self):
        """Test X..Y range returns IntegerRange when explicitly requested"""
        result = kw_converter_int('1..10', keep_range_as_string=False)
    ...
    def test_negative_range_as_string(self):
        """Test negative X..Y range as string"""
