...
class TestSmartOpen:
    ...
    def test_smart_open_invalid_mode(self):
        ...
        with pytest.raises(ValueError, match="Invalid mode"):
            with smart_open("test.txt", "invalid_mode") as f:
                pass
...
""")
        ...
        runner = CliRunner()
        result = runner.invoke(cp2kgen, [str(test_file), "global/project_name=[test1,test2]"])
...
""")
        ...
        runner = CliRunner()
        result = runner.invoke(cp2kgen, ["--zip", str(test_file), "global/project_name=[p1,p2]", "global/run_type=[ENERGY,MD]"])
