...
SERVER_ID = "cp2k-lsp-enhanced"
LANGUAGE_ID = "cp2k"
REPOSITORY = "newtontech/cp2k-lsp-enhanced"
DEFAULT_BRANCH = "develop"
RULE_PREFIX = "CP2K"
SOURCE_ROOTS = ["cp2k_input_tools"]
...
def infer_symbol(path: Path, text: str) -> str:
    for pattern in (r"^class\s+(\w+)", r"^def\s+(\w+)", r"^pub\s+(?:async\s+)?fn\s+(\w+)", r"^pub\s+struct\s+(\w+)"):
    ...
    for source_root in SOURCE_ROOTS:
        ...
        for candidate in sorted(base.rglob("*")):
            ...
            if candidate.suffix not in {".py", ".rs", ".ts", ".js"}:
                continue
            if any(part in {"__pycache__", "node_modules", "target"} for part in candidate.parts):
                continue
...
def collect_rule_ids(root: Path) -> list[dict[str, str]]:
    ...
    for path in source_files(root):
        ...
        codes.update(normalize_rule(match.group(0)) for match in SHORT_RULE_RE.finditer(text))
...
def ensure_raw_manifest(root: Path, wiki_sources: list[dict[str, str]], check: bool) -> bool:
...
def build_report(root: Path, check: bool = False) -> dict[str, Any]:
    ...
    source_urls = [
        ...
        for item in sorted(wiki_sources, key=lambda entry: (entry["rawPath"], entry["sourceUrl"]))
    ]
    report = {
...
def main() -> int:
    ...
    parser.add_argument("--check", action="store_true", help="fail if the report would change")
    ...
    if args.check:
        ...
        if current != report:
            print(f"{REPORT_PATH} is stale; run scripts/generate-traceability-report.py", file=sys.stderr)
            return 1
        return 0
