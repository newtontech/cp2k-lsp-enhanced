...
class DependencyExtractor:
    ...
    def __init__(self, base_dir="."):
        ...
        self.variable_references: Dict[str, List[Tuple[str, int, str]]] = defaultdict(list)  # var -> list of (ref_var, line, filename)
    ...
    def _process_content(self, content: str, filename: str) -> None:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            external_match = re.match(
                r"\s*(BASIS_SET_FILE_NAME|POTENTIAL_FILE_NAME|XYZ_FILE|RESTART_FILE_NAME|WFN_FILE_NAME)\s+(.+)",
                line,
                re.IGNORECASE
            )
            if external_match:
    ...
    def extract_from_dict(d, filename="tree"):
        ...
        for key, value in d.items():
            if isinstance(value, str):
                ...
                if key.upper() in ("BASIS_SET_FILE_NAME", "POTENTIAL_FILE_NAME", 
                                  "XYZ_FILE", "RESTART_FILE_NAME", "WFN_FILE_NAME"):
