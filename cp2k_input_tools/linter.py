...
# Lint rule codes
RULE_MISSPELLED_KEYWORD = "cp2k.syntax.misspelled_keyword"
RULE_INVALID_NESTING = "cp2k.syntax.invalid_nesting"
RULE_DUPLICATE_KEYWORD = "cp2k.schema.unsafe_duplicate_keyword"
...
def lint_missing_end(text: str) -> List[Diagnostic]:
    ...
    for i, line in enumerate(lines):
        ...
        # Skip empty lines and comments
        if not stripped or stripped.startswith("!") or stripped.startswith("#") or stripped.startswith("@"):
            # Check for unterminated sections at file end
            continue
...
def lint(text: str, base_dir: str = ".") -> List[Diagnostic]:
    ...
    all_diagnostics.extend(lint_missing_files(text, base_dir))
