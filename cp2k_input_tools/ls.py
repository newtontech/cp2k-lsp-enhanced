...
def _get_keyword_info(keyword_name: str, section_name: Optional[str] = None) -> Optional[dict]:
...
def _build_document_symbols(text: str) -> List[DocumentSymbol]:
    ...
    stack: List[Tuple[DocumentSymbol, List[DocumentSymbol]]] = []  # (symbol, children_list)
    ...
    for i, line in enumerate(lines):
        ...
        if sec_match:
            ...
            stack.append((symbol, symbol.children if symbol.children is not None else []))
