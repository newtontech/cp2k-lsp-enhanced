...
SKILL_SPEC_JSON = (
    '{"schema":"scientific-lsp-skill/v1","name":"cp2k","software":"cp2k","display'
    '_name":"CP2K","description":"CP2K input preflight for generated .inp files a'
    'nd runtime logs.","package":{"name":"cp2k-lsp-enhanced","install":"pip insta'
    'll cp2k-lsp-enhanced"},"entrypoints":{"server":"cp2k-language-server","tool"'
    ':"cp2k-lsp-tool"},"file_patterns":["*.inp"],"operations":["capabilities","ch'
    'eck","context","complete","hover","symbols","fix"],"diagnostic_contract":"Di'
    'agnosticEnvelope/v1","blocking_policy":{"mode":"fail-on-blocking","descripti'
    'on":"Use --fail-on-blocking when generated inputs must be launch-ready."},"s'
    'ource_provenance":[{"kind":"official_docs","label":"CP2K executable/version-'
    'matched manual generation","url":"https://www.cp2k.org/input_file"},{"kind":'
    '"official_docs","label":"CP2K latest input reference","url":"https://manual.'
    'cp2k.org/trunk/CP2K_INPUT.html"}]}'
)
...
SKILL_MD_LINES = [
    ...
    '',
    (
        'Use this skill when preparing, repairing, or reviewing CP2K input files befo'
    ...
    '',
    (
        'This installs the `cp2k-language-server` language server and the `cp2k-lsp-t'
        'ool` agent CLI from the `cp2k-lsp-enhanced` Python package.'
    ),
    '',
    ...
    (
        'cp2k-lsp-tool context <input-file-or-dir> --line 0 --character 0 --format js'
        'on'
    ),
    'cp2k-lsp-tool hover <input-file-or-dir> --line 0 --character 0 --format json',
    (
        'cp2k-lsp-tool complete <input-file-or-dir> --line 0 --character 0 --format j'
        'son'
    ),
