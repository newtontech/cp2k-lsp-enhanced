...
from lsprotocol.types import Position, Range, WorkspaceEdit, TextDocumentEdit, TextEdit
...
def _rename_section_parameter(
    ...
    # Create the edit
    edit_range = Range(
        start=Position(line=line_num, character=param_start),
        end=Position(line=line_num, character=param_end)
    )
    
    text_edit = TextEdit(range=edit_range, new_text=new_name)
    document_edit = TextDocumentEdit(
        text_document=dict(uri=uri, version=None),
        edits=[text_edit]
    )
